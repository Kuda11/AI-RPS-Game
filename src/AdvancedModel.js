  ...
  renderPredictions = (predictions, labels) => {
    if (this.canvasRef.current) {
      ...
      predictions.forEach(prediction => {
        ...
        return this.props.stopGame ? null : this.props.rpsGame(label) 
      })
    }
  ...
  render() {
    ...
    return (
      <div className="advancedContainer">
        {this.state.loading && (
          <p id="advancedLoadText">Please wait, loading game</p>
        )}       
        <video
        ...
        />
        <canvas
          className={`advancedBox ${this.props.stopGame ? 'stop-game' : ''}`}
          ref={this.canvasRef}
          width="600"
          height="500"
        />
